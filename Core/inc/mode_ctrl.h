...
/// @brief 模式状态描述对象（表驱动 FSM 使用）。
typedef struct
{
    const char *name;                 ///< 模式名称（调试/日志用）
    void (*enter)(Mode_Ctx_t *ctx);   ///< 进入模式回调
    void (*execute)(Mode_Ctx_t *ctx); ///< 周期执行回调
    void (*exit)(Mode_Ctx_t *ctx);    ///< 退出模式回调
} Mode_State_t;

/// @brief 顶层模式有限状态机对象。
typedef struct
{
    const Mode_State_t *current; ///< 当前模式
    const Mode_State_t *next;    ///< 目标模式（非 NULL 表示有切换请求）
} Mode_FSM_t;

/// @brief 命令数据包（入队对象）。
typedef struct
{
    Mode_CommandId_t cmdId;           ///< 命令 ID
    Mode_CommandSource_t source;      ///< 命令来源
    Mode_Id_t targetMode;             ///< 目标模式（仅在 SWITCH_MODE 命令时有效）
    int32_t i32Payload;               ///< 32 位整型载荷（预留）
    float f32Payload;                 ///< 32 位浮点载荷（位置/压力命令用于百分比 0~100）
} Mode_CommandPacket_t;

/// @brief 固定长度命令队列（环形缓冲）。
typedef struct
{
    Mode_CommandPacket_t items[MODE_CTRL_CMD_QUEUE_SIZE]; ///< 队列缓存
    volatile uint8_t head;                                ///< 写指针
    volatile uint8_t tail;                                ///< 读指针
    volatile uint8_t count;                               ///< 当前元素数量
    volatile uint16_t dropped;                            ///< 丢弃计数（队列满时递增）
} Mode_CommandQueue_t;

/// @brief 模式切换追踪记录（用于诊断）。
typedef struct
{
    uint32_t tick0p1ms;           ///< 发生时刻（0.1ms tick）
    Mode_Id_t fromMode;           ///< 源模式
    Mode_Id_t toMode;             ///< 目标模式
    Mode_CommandId_t byCommand;   ///< 触发命令
    Mode_CommandSource_t source;  ///< 命令来源
    Mode_SwitchReject_t rejectReason; ///< 拒绝原因（allowed=0 时有效）
    uint8_t allowed;              ///< 是否允许（1=允许，0=拒绝）
} Mode_TransitionTrace_t;

/// @brief 模式切换追踪环。
typedef struct
{
    Mode_TransitionTrace_t items[MODE_CTRL_TRACE_DEPTH]; ///< 追踪缓存
    uint8_t head;                                        ///< 写指针
    uint8_t count;                                       ///< 有效数量
} Mode_TraceRing_t;
...
typedef struct
{
    ...
    volatile uint32_t lastPositionQueryTick; ///< 上次位置模式查询 tick
