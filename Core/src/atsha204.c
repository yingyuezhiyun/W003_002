...
static bool Sha_WaitTxReady(uint32_t timeout)
{
    while (timeout > 0U)
    {
        ...
        if ((sts & I2C_STS_NO_ACK) != 0U) { Sha_ClearI2CStatus(); return false; }
        if (Sha_IsFifoEnabled())
        {
            if (I2C_getTxFIFOStatus(ATSHA204_I2C_BASE) != I2C_FIFO_TXFULL) return true;
        }
        else if ((sts & I2C_STS_TX_DATA_RDY) != 0U) return true;
        timeout--;
    }
    return false;
}
...
static bool Sha_WaitRxReady(uint32_t timeout)
{
    while (timeout > 0U)
    {
        ...
        if (Sha_IsFifoEnabled())
        {
            if (I2C_getRxFIFOStatus(ATSHA204_I2C_BASE) != I2C_FIFO_RXEMPTY) return true;
        }
        else if ((sts & I2C_STS_RX_DATA_RDY) != 0U) return true;
        timeout--;
    }
    return false;
}
...
{
    uint16_t i;
    if (!Sha_WaitBusIdle(ATSHA204_I2C_TIMEOUT)) return false;
    ...
    for (i = 0U; i < len; ++i)
    {
        ...
        { I2C_sendStopCondition(ATSHA204_I2C_BASE); return false; }
...
static bool Sha_I2C_ReadBytes(uint16_t devAddr, uint8_t *buf, uint16_t len)
{
    ...
    for (i = 0U; i < len; ++i)
    {
        ...
        { I2C_sendStopCondition(ATSHA204_I2C_BASE); (void)Sha_WaitStopDone(ATSHA204_I2C_TIMEOUT); return false; }
...
static uint16_t ATSHA204_Crc16(const uint8_t *data, uint16_t len)
{
    ...
    for (i = 0U; i < len; ++i)
    {
        ...
        for (j = 0U; j < 8U; ++j)
        {
            if (crc & 0x8000U) crc = (crc << 1U) ^ 0x8005U;
            else               crc <<= 1U;
        }
    }
    return crc;
                                  ...
                                  uint32_t execDelayUs)
{
    ...
    if (count > sizeof(cmdBuf)) return false;
    ...
    {
        uint16_t i;
        for (i = 0U; i < txDataLen; ++i) cmdBuf[idx++] = txData[i];
    }
    ...
    if (!Sha_I2C_WriteBytes(ATSHA204_I2C_ADDR, cmdBuf, idx)) return false;
    ...
    /* 读取响应：[count][data...][crc_lo][crc_hi] */
    {
        ...
        if (respTotalLen > sizeof(respBuf)) return false;
        if (!Sha_I2C_ReadBytes(ATSHA204_I2C_ADDR, respBuf, respTotalLen)) return false;

        if (respBuf[0] != respTotalLen) return false;
        ...
        /* 校验 CRC */
        {
            ...
            if (respCrc != recvCrc) return false;
        }
                                             ...
                                             uint32_t execDelayUs)
{
    ...
    {
        uint8_t respBuf[4];
        if (!Sha_I2C_ReadBytes(ATSHA204_I2C_ADDR, respBuf, 4U)) return false;
        if (respBuf[0] != 0x04U) return false;
        /* status == 0x00 表示成功 */
...
{
    if (slot > 15U || data == NULL) return false;
...
bool HWBind_ComputeMAC(const DSP_UniqueId_t *dspUID, uint8_t macOut[ATSHA204_MAC_LEN])
{
    ...
    if (dspUID == NULL || macOut == NULL) return false;
...
HW_BindResult_t HWBind_Verify(void)
{
    ...
    /* 检查 Slot 1 是否已授权（非全 0xAA 表示已写入） */
    {
        ...
        {
            if (storedSlot[i] != 0xAAU) { allAA = false; break; }
        }
        if (allAA)
        {
            ATSHA204_Sleep();
