...
SET_OBJECT_PARAMS(set_ECAT_Alias_param, glob_value.paramCfg.ECAT_Alias)
SET_OBJECT_PARAMS(set_ECAT_ID_param, glob_value.paramCfg.ECAT_ID)
...
static Command_t commands[] = {
    ...
    CMD_PARAM_ENTRY("PSE", set_Pos_limit_Open_Backoff_param),                                // 设置标定过程 全开位置回退量
    CMD_PARAM_ENTRY("PSF", set_pressctrl_MinBaseStep_param),                                 // 设置压力控制 上升小K过程值
    ...
    CMD_PARAM_ENTRY("PSN", set_ECAT_Alias_param),                                            // 设置 EtherCAT 别名
    CMD_PARAM_ENTRY("PSO", set_ECAT_ID_param),                                               // 设置 EtherCAT ID
    ...
    CMD_READ_FLOAT("PRE", "Open_Backoff+%.2f", glob_value.paramCfg.Pos_limit.Open_Backoff),  // 读取标定过程 全开位置回退量
    CMD_READ_INT32("PRF", "MinBaseStep+%ld", g_lMinBaseStep),                                // 读取压力控制 上升小K过程值
